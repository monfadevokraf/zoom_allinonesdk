...
Future<void> checkAndDownloadSDK(String location) async {
  print('checkAndDownloadSDK');
  var iosSDKFile =
      '$location/ios/MobileRTC.xcframework/ios-arm64_armv7/MobileRTC.framework/MobileRTC';
  ...
  if (!exists) {
    print('androidCommonLibFile !exists');
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/scl/fi/6k9cesun5mwel3mjycsjm/commonlib.aar?rlkey=ouabvs6c9l4rw2sla7a8u2hur&st=hswpoizj&dl=1'),
        androidCommonLibFile);
  }
  var androidRTCLibFile = '$location/android/libs/mobilertc.aar';
  exists = await File(androidRTCLibFile).exists();
  if (!exists) {
    print('androidRTCLibFile !exists');
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/scl/fi/i6z0phdp86gs8ime6ukpw/mobilertc.aar?rlkey=xus19sp8ri2qf8qbvy2bxyrsv&st=zjtlm298&dl=1'),
        androidRTCLibFile);
  }
}
